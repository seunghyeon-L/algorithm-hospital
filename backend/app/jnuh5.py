...
# 소요시간(mean·sd): 수술시간 문헌의 분포 형태를 유지하되, 각 과 평균이 제주대병원
# 정보공개청구 답변서 표6(2025 과별 평균 집도시간 = 총집도시간÷건수)과 일치하도록 보정.
#   외과107·정형134·산부98·안과41·신경159·이비126·비뇨97·흉부171·성형71분.
# (분위수는 미제공 → 분포 형태는 문헌 유지.) ktas는 수술 긴급도 매핑.
JNUH5_SURGERY_TYPES: Dict[str, List[Tuple[str, int, int, int, int]]] = {
    "surg_gs": [("충수절제술", 69, 25, 2, 1), ("담낭절제술", 103, 30, 2, 3),
                ("탈장교정술", 77, 24, 2, 5), ("대장절제술", 179, 71, 3, 3)],
    "surg_os": [("슬관절치환술", 105, 42, 3, 5), ("고관절치환술", 125, 45, 3, 3),
                ("골절정복술", 173, 91, 2, 2)],
    "surg_ns": [("추간판절제술", 141, 52, 3, 3), ("개두술", 177, 65, 3, 1)],
    "surg_obgy": [("제왕절개", 80, 22, 2, 1), ("자궁절제술", 116, 44, 2, 3)],
    "surg_oph": [("백내장수술", 23, 8, 1, 5), ("유리체절제술", 59, 20, 2, 4)],
    "surg_ent": [("편도절제술", 84, 28, 2, 5), ("부비동내시경수술", 168, 56, 2, 5)],
    "surg_uro": [("경요도절제술", 105, 45, 2, 5), ("요로결석제거술", 89, 33, 2, 3)],
    "surg_cs": [("폐엽절제술", 171, 57, 3, 2)],
    "surg_ps": [("피판재건술", 71, 32, 2, 4)],
}
